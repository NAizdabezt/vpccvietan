...
function ReconScreen() {
  ...
  const { Table, Badge, Button, Select } = window.FSICheckinDesignSystem_019df8;
  const cash = D.receipts.filter((r) => r.pay === "cash").reduce((s, r) => s + r.amount, 0);
  const transfer = D.receipts.filter((r) => r.pay === "transfer").reduce((s, r) => s + r.amount, 0);
      ...
      <div style={{ maxWidth: "var(--content-max)", margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-tertiary)", margin: "2px 0 0" }}>Bảng kê biên lai · {D.shift} · {D.today}</p>
          </div>
          <div style={{ width: 160 }}><Select options={["Ca sáng", "Ca chiều", "Cả ngày"]} defaultValue="Ca sáng" /></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          ...
          <MiniStat label="Số biên lai" value={String(D.receipts.length)} />
        </div>

        <Table columns={columns} rows={D.receipts} />
...
function MergeGroupCard({ g }) {
  const L = window.LucideReact;
  const { fmtVND, fullSoCC } = window.POSFmt;
  const { Button } = window.FSICheckinDesignSystem_019df8;
  const [sel, setSel] = useStateA(() => g.items.map((it) => it.no));
  const [merged, setMerged] = useStateA(false);
  const toggle = (no) => setSel((s) => s.includes(no) ? s.filter((x) => x !== no) : [...s, no]);
  const chosen = g.items.filter((it) => sel.includes(it.no));
    ...
    <div style={{ border: "1px solid var(--border-default)", background: "var(--bg-surface)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", background: "var(--bg-elevated)", borderBottom: "1px solid var(--border-subtle)" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-tertiary)" }}>{g.addr} · MST {g.taxcode}</div>
        </div>
      ...
      <div>
        {g.items.map((it, i) => {
          const on = sel.includes(it.no);
          return (
            <label key={it.no} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", borderTop: i ? "1px solid var(--border-subtle)" : "none", cursor: merged ? "default" : "pointer", background: on ? "transparent" : "var(--bg-overlay)" }}>
              <input type="checkbox" checked={on} disabled={merged} onChange={() => toggle(it.no)} style={{ width: 16, height: 16, accentColor: "var(--accent)", cursor: merged ? "default" : "pointer" }} />
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-tertiary)", width: 58 }}>{fullSoCC(it.no).split("/")[1]}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderTop: "1px solid var(--border-default)", background: "var(--bg-elevated)" }}>
        {merged ? (
          ...
          <>
            ...
            <Button variant="primary" size="sm" icon={L.Combine} disabled={chosen.length < 2} onClick={() => setMerged(true)}>Gộp &amp; xuất 1 hóa đơn</Button>
          </>
        )}
      </div>
    </div>
...
function InvoiceScreen() {
  ...
  const issuedToday = D.invoices.filter((i) => i.status === "issued");
  const errored = D.invoices.filter((i) => i.status === "error");

  const matchF = (i) => filter === "all" ? true : filter === "overdue" ? isOverdue(i) : i.status === filter;
  const visible = D.invoices.filter(matchF);
      ...
      <div style={{ maxWidth: "var(--content-max)", margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-tertiary)", margin: "2px 0 0" }}>Gộp hóa đơn · lùi ngày · liên thông {D.easyInvoice.provider} · hôm nay {D.today}</p>
          </div>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 12.5, fontWeight: 500, color: "var(--text-success)", background: "var(--bg-success)", border: "1px solid var(--border-success)", borderRadius: "var(--radius-full)", padding: "5px 12px" }}>
            <L.PlugZap size={14} /> {D.easyInvoice.provider} · đồng bộ {D.easyInvoice.lastSync.split("·")[1]}
          </span>
        ...
        {/* Gộp hóa đơn — doanh nghiệp nhiều giao dịch */}
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <L.Combine size={16} color="var(--accent)" />
            <h2 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>Gộp hóa đơn</h2>
            <span style={{ fontSize: 12.5, color: "var(--text-tertiary)" }}>· tự gom giao dịch cùng khách hàng (Tên + Địa chỉ + MST) · {D.groups.length} nhóm</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {D.groups.map((g) => <MergeGroupCard key={g.id} g={g} />)}
          </div>
        </div>
                      ...
                      <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                        {inv.status === "issued"
                          ...
                          : inv.status === "error"
                            ...
                            : <Button variant={od ? "primary" : "secondary"} size="sm" icon={L.Send}>{od ? "Xuất gấp" : "Xuất & đẩy"}</Button>}
                      </td>
                    </tr>
                  );
                })}
